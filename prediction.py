...
def print_banner():
    ...
    """)
def fetch_market_data(assets: List[str], years: int, risk_level: int) -> Tuple[pd.DataFrame, np.ndarray, np.ndarray]:
...
def main():
    ...
    # Get user inputs
    sector_name, assets, risk_level, investment_amount, years, mode = get_user_inputs()
    ...
    ibm_backend_preference = os.getenv('IBM_BACKEND')  # e.g., simulator_mps
    ibm_force_simulator = os.getenv('IBM_FORCE_SIMULATOR', 'true').strip().lower() in {"1", "true", "yes", "y"}
    # Force the free Platform channel by default
    ibm_channel = os.getenv('IBM_CHANNEL', 'ibm_quantum_platform').strip().lower()

    # Suppress noisy Qiskit warnings about instances
    ...
    # Initialize IBM Quantum service; default to Platform and avoid instance usage
    service = None
    ...
    market_data, expected_returns, risk_matrix = fetch_market_data(assets, years, risk_level)
    ...
    num_assets = len(assets)
    qc = create_quantum_portfolio_circuit(num_assets)
    ...
    try:
        if service:
            ...
            backend = None
            try:
                if ibm_backend_preference:
                    try:
                        backend = service.backend(ibm_backend_preference)
                    except Exception:
                        backend = service.get_backend(ibm_backend_preference)
                if backend is None:
                    ...
                    for name in preferred_sims:
                        try:
                            ...
                            continue
                if backend is None:
                    sims = service.backends(operational=True, simulator=True)
            ...
            with Session(service=service, backend=backend) as session:
                ...
                prediction_result = process_quantum_results(counts, assets, expected_returns, risk_matrix, pure_quantum=pure_quantum)
        ...
        if pure_quantum:
            ...
            prediction_result = process_quantum_results(counts, assets, expected_returns, risk_matrix, pure_quantum=True)
        else:
            print("🔄 Using classical optimization fallback...")
            prediction_result = classical_optimization_fallback(assets, expected_returns, risk_matrix)
...
def create_quantum_portfolio_circuit(num_assets: int) -> QuantumCircuit:
...
def process_quantum_results(counts: Dict[str, int], assets: List[str], 
    ...
    try:
        ...
        if pure_quantum:
            ...
            optimized_weights = optimize_portfolio_weights(weights, expected_returns, risk_matrix)
        ...
        quantum_risk_analysis = calculate_quantum_risk_metrics(
...
def optimize_portfolio_weights(initial_weights: np.ndarray, expected_returns: np.ndarray, 
...
def calculate_quantum_risk_metrics(
    weights: np.ndarray,
    ...
    This is a lightweight analysis to avoid runtime failures while
    still providing meaningful, easy-to-explain numbers for demos.
    """
    num_assets = len(assets)
...
def classical_optimization_fallback(assets: List[str], expected_returns: np.ndarray, 
...
if __name__ == '__main__':
    print("🚀 Q VEST - IBM Quantum Portfolio Prediction")
    print("=" * 50)
